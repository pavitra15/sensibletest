...
<?php
    if(isset($_POST['update'])) 
    {
        ...
        $date=date('Y-m-d');
        $id=$_SESSION['id'];
        $sth=$db->prepare("update device set access_control='$access_control', status_change_date='$date' where d_id='$did'");
        echo "update device set access_control='$access_control', status_change_date='$date' where id='$did'";
        if($sth->execute())
        {
            $flag=1;
        }
                            ...
                            <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                ...
                                <tbody>
                                    ...
                                    include('../connect.php');
                                        ...
                                        $query=$db->prepare("select d_id, deviceid, device_name, serial_no, first_name,last_name from device,user_mst where access_control='$access_control' and device.status='$status' and user_mst.id= device.id");
                                        $query->execute();
                                        if($data=$query->fetch())
                                        {
                                            $i=1;
                                            do 
                                            {
                                                echo'<tr>
                                                    <form action="" method="post">
                                                        ...
                                                        <td ><input type="submit" name="update" value="Deny" class="btn btn-danger"></td>
                                                    </form>
                                                </tr>';
                                            $i++;
