...
session_start();
    ...
    $incl_product_query=$db->prepare("select sum(bill_amt) as total, sum(tax_amt) as total_tax, sum(cash) as total_cash, sum(credit) as total_credit,sum(parcel_amt) as total_parcel_amt, sum(digital) as total_digital, sum(discount) as total_discount from( select DISTINCT bill_no, bill_amt, tax_amt, cash, credit, parcel_amt, digital, discount from transaction_mst where transaction_mst.device_id='$d_id' and transaction_mst.status='$status' and tax_state!=0 and bill_date between '$start_date' and '$end_date')T1");
    ...
    $product_query=$db->prepare("select sum(bill_amt) as total, sum(tax_amt) as total_tax, sum(cash) as total_cash,sum(parcel_amt) as total_parcel_amt, sum(credit) as total_credit, sum(digital) as total_digital, sum(discount) as total_discount, sum(round_off) as total_round_off from ( select DISTINCT bill_no, bill_amt, tax_amt, cash, parcel_amt, credit, digital, discount, round_off from  transaction_mst where transaction_mst.device_id='$d_id' and transaction_mst.status='$status' and bill_date between '$start_date' and '$end_date')T1");
