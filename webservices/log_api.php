    ...
    if($token_count==1)
    {
    	...
    	for($i=0;$i<$count;$i++)
    	{
	    	...
	    	if($log_type==6)
	    	{
	    		...
	    		$stock_added=$extra->stock_added;

	    		$check_query=$db->prepare("select * from stock_dtl where d_id='$d_id' and log_date='$log_date' and stock_from='$stock_from' and product_id='$product_id' and stock_added='$stock_added'");
	    		$check_query->execute();
	    		...
	    		{
		    		$query=$db->prepare("insert into stock_dtl(d_id, log_date, stock_from, product_id,stock_added) values('$d_id', '$log_date', '$stock_from', '$product_id','$stock_added')");
			        $query->execute();
			        if($query->rowCount())
			        {
			            $responce = array('status' =>1,'id'=>$id);
