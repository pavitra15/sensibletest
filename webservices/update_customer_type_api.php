    ...
    if($token_count==1)
    {
        ...
        if(json_validator($json))
        {
            ...
            for($i=0; $i<$length; $i++)
            {
                ...
                {
                    $query=$db->prepare("update customer_dtl set customer_name='$customer_name', updated_by_date='$current_date', updated_by_id='$id' where customer_id='$customer_id'");
                    $query->execute();
                    $select_query=$db->prepare("select customer_id, customer_name from customer_dtl where customer_id='$customer_id' and status='$status'");
