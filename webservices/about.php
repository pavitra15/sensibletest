	...
	if($token_count==1)
	{
		...
		if($about_count>0)
		{
			while($about_dta=$about_query->fetch())
			{
				...
				$email=$about_dta['email'];
				$contact=$about_dta['office_contact'];
				$path=$about_dta['path'];
			}
			$response='{"about":{"company_name":"'.$company_name.'","website":"'.$website.'","email":"'.$email.'","contact":"'.$contact.'","powered_by":"'.$company_name.'","path":"'.$path.'"},"device_name":';
		}
		else
		{
			$about_query = $db->prepare("select * from dealer_mst where dealer_id=1");
			$about_query->execute();
			$about_count=$about_query->rowCount();
			if($about_count>0)
			{
