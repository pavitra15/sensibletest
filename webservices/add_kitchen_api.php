            ...
            for($i=0; $i<$length; $i++)
            {
                ...
                else
                {
                    ...
                    if($r_count<5)
                    {
                        ...
                        $query=$db->prepare("insert into kitchen_dtl (kitchen_name, deviceid, status, created_by_date, created_by_id) values('$kitchen_name', '$d_id', '$status' , '$current_date', '$id')");
                        $query->execute();
                        $select_query=$db->prepare("select kitchen_id, kitchen_name from kitchen_dtl where kitchen_name='$kitchen_name' and status='$status' and deviceid='$d_id'");
