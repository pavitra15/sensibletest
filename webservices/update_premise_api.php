        ...
        if(json_validator($json))
        {
            ...
            for($i=0; $i<$length; $i++)
            {
                ...
                if(in_array($premise_name, $exists_arr))
                {
                    ...
                    {
                        $query=$db->prepare("update premise_dtl set premise_name='$premise_name', no_of_table='$no_of_table' where premise_id='$premise_id'");
                        $query->execute();
                        $select_query=$db->prepare("select premise_id, premise_name,no_of_table from premise_dtl where premise_id='$premise_id' and status='$status'");
