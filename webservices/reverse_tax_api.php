<?php
    ...
    if($token_count==1)
    {
        ...
        $status="active";
        $query=$db->prepare("update device set tax_type='$tax_type', updated_by_id='$id', updated_date='$current_date' where d_id='$d_id'");
        $query->execute();

        $device_query=$db->prepare("select d_id, tax_type from device where deviceid='$deviceid'");
        $device_query->execute();
        if($device_data=$device_query->fetch())
        {
            do
            {
                $tax_type=$device_data['tax_type'];
            }
            while($device_data=$device_query->fetch());
        }
        $responce='{"status":1,"tax_type":"'.$tax_type.'"}';
        echo $responce;
    }
    else
    {
