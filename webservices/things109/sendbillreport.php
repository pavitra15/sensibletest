			...
			if($length>0)
			{
				for($i=0; $i<$length; $i++)
				{
					...
					elseif($bill_data[$i]->bill_state==6)
					{
						...
						// $exist_bl_query=$db->prepare("select bill_no from transaction_mst where device_id='$d_id' and bill_no='$bill_no' and user_id='$user_id' and parcel_amt='$parcel_amt' and tax_amt='$tax_amt'  and bill_date='$date'");
				  ...
				  //       }
		                ...
		                {
		                	$status='active';
							$query=$db->prepare("insert into transaction_mst (device_id,bill_no,user_id,parcel_amt,tax_amt,bill_amt,tax_state,discount, cash, credit, digital, customer_id, waiter_id, bill_type_id, bill_date,sync_by,created_sync_date,status,settled) values('$d_id','$bill_no','$user_id','$parcel_amt','$tax_amt','$bill_amt','$tax_state', '$discount','$cash','$credit','$digital', '$customer_id','$waiter_id', '$bill_type_id', '$date','$sync_by','$created_sync_date','$status','$settled')");
							$query->execute();
							$rs=$query->rowCount();
							if($rs>0)
							{
