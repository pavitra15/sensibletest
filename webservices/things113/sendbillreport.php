<?php
	session_start();
	include('../../connect.php');
	if($_SESSION['execution']==1 || !isset($_SESSION['execution']))
	{
		$_SESSION['execution']=0;
		$token=$_POST['token'];
					...
					for($i=0; $i<$length; $i++)
					{
						...
						// {
							if($bill_data[$i]->bill_state==3)
							{
								...
								$settled=$bill_data[$i]->settled;
								$bill_amt=$bill_data[$i]->bill_amt;
								$tax_amt=$bill_data[$i]->tax_amt;

								$round_off=$bill_data[$i]->round_off;
								...
								$update_date=$bill_data[$i]->update_date;
								$bill_no=$bill_data[$i]->bill_no;
								...
								$query=$db->prepare("update transaction_mst set parcel_amt='$parcel_amt', round_off='$round_off', discount='$discount', tax_amt='$tax_amt', bill_amt='$bill_amt', tax_state='$tax_state', cash='$cash', credit='$credit', digital='$digital', customer_id='$customer_id', waiter_id='$waiter_id', bill_type_id='$bill_type_id', updated_by='$user_id', updated_date='$update_date',updated_sync_by='$sync_by', updated_sync_date='$created_sync_date' where device_id='$d_id' and bill_no='$bill_no'");
								if($query->execute())
								{
									$response.='{"bill_no":"'.$bill_no.'","status":1},';		
							...
							elseif($bill_data[$i]->bill_state==6)
							{
								...
								$query_count=$exist_bl_query->rowCount();
						        if($query_count==0)
				                {
				                	$status='active';
									$query=$db->prepare("insert into transaction_mst (device_id,bill_no,user_id,parcel_amt,round_off, tax_amt,bill_amt,tax_state,discount, cash, credit, digital, customer_id, waiter_id, bill_type_id, bill_date,sync_by,created_sync_date,status,settled) values('$d_id','$bill_no','$user_id','$parcel_amt','$round_off','$tax_amt','$bill_amt','$tax_state', '$discount','$cash','$credit','$digital', '$customer_id','$waiter_id', '$bill_type_id', '$date','$sync_by','$created_sync_date','$status','$settled')");
									$query->execute();
									$rs=$query->rowCount();
									if($rs>0)
									{
	...
	{
		$responce = array('status' =>3,'message'=>'Already executing');
		echo json_encode($responce);
	}
?>