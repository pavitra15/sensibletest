			...
			if($length>0)
			{
				for($i=0; $i<$length; $i++)
				{
					...
					elseif($bill_data[$i]->bill_state==6)
					{
		                ...
		                {
		                	$status='active';
							$query=$db->prepare("insert into transaction_mst (device_id,bill_no,user_id,parcel_amt,tax_amt,bill_amt,cash, credit, digital, customer_id, waiter_id, bill_date,sync_by,created_sync_date,status) values('$d_id','$bill_no','$user_id','$parcel_amt','$tax_amt','$bill_amt','$cash','$credit','$digital', '$customer_id','$waiter_id', '$date','$sync_by','$created_sync_date','$status')");
							$query->execute();
							$rs=$query->rowCount();
							if($rs>0)
							{
