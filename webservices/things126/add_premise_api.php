        ...
        if(json_validator($json))
        {
            ...
            for($i=0; $i<$length; $i++)
            {
                ...
                else
                {
                    ...
                    if($r_count<5)
                    {
                        $exists_arr[]=$premise_name;
                        $query=$db->prepare("insert into premise_dtl (premise_name, no_of_table, table_range,premise_type,deviceid, status, created_by_date, created_by_id) values('$premise_name', '$no_of_table', '$table_range','$premise_type',  '$d_id', '$status' , '$current_date', '$id')");
                        $query->execute();
                        $select_query=$db->prepare("select premise_id, premise_name,no_of_table, table_range,premise_type from premise_dtl where premise_name='$premise_name' and no_of_table='$no_of_table' and status='$status' and deviceid='$d_id'");
