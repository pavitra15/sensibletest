            ...
            for($i=0; $i<$length; $i++)
            {
                ...
                if($r_count<5)
                {
                    if($user_type=="1")
                    {
                        ...
                        {
                            $exists_admin[]=$user_name;
                            $admin_arr[]=$user_type;
                            $query=$db->prepare("insert into user_dtl (user_name, user_type, user_mobile,deviceid, status, created_by_date, created_by_id) values('$user_name', '$user_type', '$user_mobile', '$d_id', '$status' , '$current_date', '$id')");
                            $query->execute();
                            $select_query=$db->prepare("select user_id, user_name, user_type, user_mobile from user_dtl where user_name='$user_name' and user_type='$user_type' and user_mobile='$user_mobile' and status='$status' and deviceid='$d_id'");
