    ...
    if($token_count==1)
    {
        ...
        if(json_validator($json))
        {
            ...
            for($i=0; $i<$length; $i++)
            {
                ...
                $status="delete";
                $query=$db->prepare("update customer_dtl set status='$status', status_change_date='$current_date', deleted_by_id='$id' where customer_id='$customer_id'");
                $query->execute();
                $response.='{"customer_id":'.$customer_id.',"status":"delete"},';
            }
