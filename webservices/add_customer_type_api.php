<?php
    ...
    if($token_count==1)
    {
        ...
        {
            $exists_arr= array();
            $push_query=$db->prepare("select * from customer_dtl where deviceid='$d_id' and status='active'");
            $push_query->execute();
            ...
            for($i=0; $i<$length; $i++)
            {
                $customer_name=ucwords($records[$i]->customer_name);
                ...
                else
                {
                    ...
                    if($r_count<5)
                    {
                        ...
                        $query=$db->prepare("insert into customer_dtl (customer_name, deviceid, status, created_by_date, created_by_id) values('$customer_name', '$d_id', '$status' , '$current_date', '$id')");
                        $query->execute();
                        $select_query=$db->prepare("select customer_id, customer_name from customer_dtl where customer_name='$customer_name' and status='$status' and deviceid='$d_id'");
