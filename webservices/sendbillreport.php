<?php
	...
	{
		file_put_contents('gs://nth-weft-193604.appspot.com/api/my.txt', $json);
			...
			if($length>0)
			{
				for($i=0; $i<$length; $i++)
				{
					if($bill_data[$i]->bill_state==3)
					{
						...
						$query=$db->prepare("update transaction_mst set parcel_amt='$parcel_amt', tax_amt= '$tax_amt', bill_amt='$bill_amt', updated_by='$user_id', updated_date='$date',updated_sync_by='$sync_by', updated_sync_date='$created_sync_date' where device_id='$d_id' and bill_no='$bill_no'");
						if($query->execute())
						{
							...
							{	
								...
								{ 
									...
									{
										$status='cancel';
										$st=$db->prepare("update transaction_dtl set quantity='$quantity',price='$price',tax_index='$tax_index',total_amt='$total', status='$status' where transaction_id='$transaction_id' and item_id='$item_id'");
										$st->execute();
										$rs=$st->rowCount();
									}
									else
					...
					elseif($bill_data[$i]->bill_state==6)
					{
		                ...
		                {
		                	$status='active';
							$query=$db->prepare("insert into transaction_mst (device_id,bill_no,user_id,parcel_amt,tax_amt,bill_amt,bill_date,sync_by,created_sync_date,status) values('$d_id','$bill_no','$user_id','$parcel_amt','$tax_amt','$bill_amt','$date','$sync_by','$created_sync_date','$status')");
							$query->execute();
							$rs=$query->rowCount();
							if($rs>0)
							{
