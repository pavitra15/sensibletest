<?php
    ...
    if($token_count==1)
    {
        ...
        if(json_validator($json))
        {
            ...
            for($i=0; $i<$length; $i++)
            {
                $waiter_name=ucwords($records[$i]->waiter_name);
                $waiter_mobile=$records[$i]->waiter_mobile;
                if(in_array($waiter_name, $exists_waiter))
                {
                }
                else
                {
                    $exists_waiter[]=$waiter_name;
                    $query=$db->prepare("insert into waiter_dtl (waiter_name, waiter_mobile,deviceid, status, created_by_date, created_by_id) values('$waiter_name', '$waiter_mobile', '$d_id', '$status' , '$current_date', '$id')");
                    $query->execute();
                    $select_query=$db->prepare("select waiter_id, waiter_name, waiter_mobile from waiter_dtl where waiter_name='$waiter_name' and waiter_mobile='$waiter_mobile' and status='$status' and deviceid='$d_id'");
                    $select_query->execute();
                    if($data=$select_query->fetch())
                    {
                        do
                        {
                            $response.='{"waiter_id":'.$data['waiter_id'].',';
                            $response.='"waiter_name":"'.$data['waiter_name'].'",';
                            $response.='"waiter_mobile":"'.$data['waiter_mobile'].'"},';
                        }
