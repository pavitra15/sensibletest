		...
		if(json_validator($json))
		{
            ...
            for($m=0; $m<$length; $m++)
            {
                ...
                else
                {
					...
					$stock_query=$db->prepare("update stock_mst set stockable='$stockable', unit_id='$unit', current_stock=current_stock+'$stock_added', reorder_level='$reorder_level' where product_id='$product_id'");
					$stock_query->execute();
