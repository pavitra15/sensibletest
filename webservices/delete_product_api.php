    ...
    if($token_count==1)
    {
        ...
        if(json_validator($json))
        {
            ...
            for($i=0; $i<$length; $i++)
            {
                ...
                $status="delete";
               $query=$db->prepare("update product set status='$status', status_change_date='$current_date', deleted_by_id='$id' where product_id='$product_id'");
                $query->execute();
                $response.='{"product_id":'.$product_id.',"status":"delete"},';
            }
