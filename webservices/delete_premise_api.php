    ...
    if($token_count==1)
    {
        ...
        if(json_validator($json))
        {
            ...
            for($i=0; $i<$length; $i++)
            {
                ...
                $status="delete";
                $query=$db->prepare("update premise_dtl set status='$status', status_change_date='$current_date', deleted_by_id='$id' where premise_id='$premise_id'");
                $query->execute();
                $response.='{"premise_id":'.$premise_id.',"status":"delete"},';
            }
