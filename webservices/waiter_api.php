<?php
	...
	if($token_count==1)
	{
	    ...
	    $st=$db->prepare("select * from waiter_dtl where deviceid='$d_id' and status='$status'");
		$st->execute();
		if($da=$st->fetch())
		{
			$user='{"waiter":[';
			do
			{
				$user.='{"waiter_id":"'.$da['waiter_id'].'",';
				$user.='"waiter_name":"'.$da['waiter_name'].'",';
				$user.='"waiter_mobile":"'.$da['waiter_mobile'].'"},';
			}	
			while ($da=$st->fetch());
		}
		$user = rtrim($user, ',');
