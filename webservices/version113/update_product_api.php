            ...
            for($m=0; $m<$length; $m++)
            {
				...
				if(in_array($english_name, $exists_arr))
                {
		            ...
		            if($count==$english_name)
		            {
						...
						$stock_query=$db->prepare("update stock_mst set stockable='$stockable', reorder_level='$reorder_level', current_stock=current_stock+'$stock_added', unit_id='$unit_id' where product_id='$product_id'");
						$stock_query->execute();
