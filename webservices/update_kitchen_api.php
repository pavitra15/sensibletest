    ...
    if($token_count==1)
    {
        ...
        if(json_validator($json))
        {
            ...
            for($i=0; $i<$length; $i++)
            {
                ...
                {
                    $query=$db->prepare("update kitchen_dtl set kitchen_name='$kitchen_name', updated_by_date='$current_date', updated_by_id='$id' where kitchen_id='$kitchen_id'");
                    $query->execute();
                    $select_query=$db->prepare("select kitchen_id, kitchen_name from kitchen_dtl where kitchen_id='$kitchen_id' and status='$status'");
