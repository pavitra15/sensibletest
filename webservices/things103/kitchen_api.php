<?php
	...
	if($token_count==1)
	{
	    ...
	    $st=$db->prepare("select * from kitchen_dtl where deviceid='$d_id' and status='$status'");
		$st->execute();
		if($da=$st->fetch())
		{
			$user='{"kitchen":[';
			do
			{
				...
				$user.='"kitchen_printer_ip":"'.$da['kitchen_printer_ip'].'",';
				$user.='"kitchen_printer_port":'.$da['kitchen_printer_port'].',';
				$user.='"printer_type":'.$data['printer_type'].',';
                $user.='"path":"'.$data['path'].'",';
                $user.='"paper_size":'.$data['paper_size'].',';
