    ...
    if($token_count==1)
    {
    	...
    	for($i=0;$i<$count;$i++)
    	{
	    	...
	    	if($log_type==6)
	    	{
	    		...
	    		{
		    		$query=$db->prepare("insert into stock_dtl(d_id, log_date, stock_from, product_id,stock_added, current_stock,opening_stock) values('$d_id', '$log_date', '$stock_from', '$product_id','$added_stock','$current_stock','$opening_stock')");
			        $query->execute();
			        if($query->rowCount())
			        {
			            $responce = array('status' =>1,'id'=>$id);
