<?php
	...
	if($token_count==1)
	{
		...
		{
			$exists_arr= array();
	        $push_query=$db->prepare("select * from product, price_mst, stock_mst where deviceid='$d_id' and status='active' and product.product_id= price_mst.product_id and product.product_id=stock_mst.product_id");
	        $push_query->execute();
            ...
            for($m=0; $m<$length; $m++)
            {
                ...
                {
					$query=$db->prepare("insert into product (regional_name, english_name,barcode, weighing, category_id,kitchen_id, deviceid, product_type,  status, created_by_date, created_by_id) values('$regional_name', '$english_name', '$barcode', '$weightable', '$category_id', '$kitchen_id', '$d_id', '$product_type', '$status' , '$created_by_date', '$id')");
	            	$query->execute();
	            	...
	            	if($data=$select_query->fetch())
	            	{
	                	...
	                	{
	                		$date = new DateTime("now", new DateTimeZone('Asia/Kolkata'));
	                		$updated_date=$date->format('Y-m-d h:m:s');
	                		$stock_query=$db->prepare("insert into stock_mst(product_id,current_stock,stockable,unit_id,updated_date)values($product_id,0,'$stockable',$unit_id,'$updated_date')");
	                		$stock_query->execute();
