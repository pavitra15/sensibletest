	...
	if($token_count==1)
	{
		...
		if(json_validator($json))
		{
            ...
            for($m=0; $m<$length; $m++)
            {
				...
				$stock_added=$records[$m]->stock_added;
				$transaction_id=$records[$m]->transaction_id;
				...
				if(in_array($english_name, $exists_arr))
                {
		            ...
		            if($count==$english_name)
		            {
						...
						$stock_query=$db->prepare("update stock_mst set stockable='$stockable', unit_id='$unit_id', current_stock=current_stock+'$stock_added', reorder_level='$reorder_level' where product_id='$product_id' and transaction_id!='$transaction_id'");
						$stock_query->execute();
                ...
                else
                {
					...
					$stock_query=$db->prepare("update stock_mst set stockable='$stockable', unit_id='$unit', current_stock=current_stock+'$stock_added', reorder_level='$reorder_level' where product_id='$product_id' and transaction_id!='$transaction_id'");
					$stock_query->execute();
