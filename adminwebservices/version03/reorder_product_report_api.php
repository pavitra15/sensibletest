<?php
    ...
    if($token_count==1)
    {
        ...
        $query=$db->prepare("select english_name,regional_name,category_name,current_stock from product,category_dtl,stock_mst,price_mst where stock_mst.product_id=product.product_id  and product.deviceid='$d_id' and product.product_id= price_mst.product_id and product.product_id=stock_mst.product_id and product.category_id= category_dtl.category_id and stock_mst.stockable='Yes' and stock_mst.current_stock <= stock_mst.reorder_level limit $start, $record");
        $query->execute();
        $data=$query->fetchAll(PDO::FETCH_ASSOC);

        $total_page=ceil($total_records/40);
        $responce = array('status' =>1 ,'current_page'=>$page_no, 'total_records'=>$total_records, 'total_page'=>$total_page,'reorder_stock_report' =>$data);   
