<?php
    ...
    if($token_count==1)
    {
        $row_query=$db->prepare("select DISTINCT bill_no, transaction_id, bill_amt, bill_date, user_name from  transaction_mst, user_dtl where transaction_mst.user_id=user_dtl.user_id and transaction_mst.device_id='$d_id' and transaction_mst.status='$status' and bill_date between '$start_date' and '$end_date'");

        $row_query->execute();
        $total_records=$row_query->rowCount();

        $query=$db->prepare("select sum(bill_amt) as total,sum(tax_amt) as total_tax, sum(cash) as total_cash, sum(credit) as total_credit, sum(digital) as total_digital , sum(discount) as total_discount,  sum(round_off) as total_round_off,  sum(parcel_amt) as total_parcel_amt, user_name, user_id from( select distinct bill_no, transaction_mst.user_id, bill_amt, tax_amt,cash, credit, digital,discount, round_off, parcel_amt,user_name from  transaction_mst, user_dtl where transaction_mst.user_id=user_dtl.user_id and transaction_mst.device_id='$d_id' and transaction_mst.status='$status' and bill_date between '$start_date' and '$end_date') T1 group by user_id");
        $query->execute();
        ...
        $data_array = array('status' =>1 ,'current_page'=>$page_no, 'total_records'=>$total_records, 'total_page'=>$total_page,'bill_summary'=>$summary_data,'bill_data'=>$detail_data);
