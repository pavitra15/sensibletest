<?php
    ...
    if($token_count==1)
    {
        $query=$db->prepare("select customer_name,customer_contact, sum(credit) AS total from( select distinct bill_no, customer_name, customer_contact, customer_id,credit from customer_mst, transaction_mst where customer_mst.customer_id=transaction_mst.customer_id and transaction_mst.status='$status' and customer_mst.deviceid='$d_id') T1 group by (customer_id)");
        $query->execute();
