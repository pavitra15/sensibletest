<?php
    ...
    if($token_count==1)
    {
        $query=$db->prepare("select english_name,regional_name, sum(transaction_dtl.quantity) AS total, sum(transaction_dtl.total_amt) as total_amt from product, transaction_dtl,transaction_mst where transaction_dtl.transaction_id=transaction_mst.transaction_id and product.product_id=transaction_dtl.item_id and transaction_mst.device_id='$d_id' and transaction_mst.status='$status' and bill_date between '$start_date' and '$end_date'  and transaction_dtl.status='active' group by (product.product_id)");
        $query->execute();
        $data=$query->fetchAll(PDO::FETCH_ASSOC);
        $responce = array('status' =>1 ,'product_report' =>$data);   
