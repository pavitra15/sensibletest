<?php
    ...
    if($token_count==1)
    {
        $query=$db->prepare("select waiter_name, sum(total_amt*comission/100) as total from waiter_dtl, transaction_mst, product, transaction_dtl where transaction_mst.waiter_id=waiter_dtl.waiter_id and waiter_dtl.deviceid='$d_id' and transaction_mst.status='active' and transaction_mst.transaction_id=transaction_dtl.transaction_id and transaction_dtl.item_id=product.product_id and bill_date between '$start_date' and '$end_date' group by waiter_dtl.waiter_id ");
        $query->execute();
        $data=$query->fetchAll(PDO::FETCH_ASSOC);
        $responce = array('status' =>1 ,'waiter_report' =>$data);   
