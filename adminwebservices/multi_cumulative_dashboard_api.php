<?php
    ...
    if($token_count==1)
    {
        ...
        $product_name="";
        $device_name="";
        $total_query=$db->prepare("select sum(bill_amt) as total, count(bill_no) as count from transaction_mst, device where transaction_mst.device_id=device.d_id and transaction_mst.status='$status' and device.id='login_id' and bill_date between '$start_date' and '$end_date'");
        $total_query->execute();
        ...
        $total_report = array('total_amount' => $total,'checkout' => $checkout,'product_name'=>$product_name,'device_name'=>$device_name);
       	...
       	$device_query=$db->prepare("select d_id, device_name, sum(transaction_mst.bill_amt) as total from device, transaction_mst where transaction_mst.device_id=device.d_id and  device.id='login_id' and bill_date between '$start_date' and '$end_date' group by(device.d_id) ");
