            ...
            <div class="row card clearfix">
                <div class="body">
                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                        <section>
                            <p>
                                By deleting device you can lose all device data. please verify your mobile OTP before delete device, its mandatory.
                            </p>
                        </section>
                        </br>
                        <fieldset>
                        ...
                        <button class="btn btn-primary waves-effect" id="generate_otp" disabled='disabled'>Generate OTP</button>
                    </div>
                    <div id="otp" class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="display: none" >
                        <div class="input-group">
                            <span class="input-group-addon">
                                <i class="material-icons">lock</i>
                            </span>
                            <div class="form-line">
                                <input type="password" class="form-control" id="mobile_otp" minlength="6" placeholder="Enter Mobile OTP" >
                            </div>
                        </div>
                        <button class="btn btn-lg bg-pink waves-effect" id="verify_otp" disabled='disabled'>VERIFY OTP</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        $('#generate_otp').click(function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    var status=sk.status;
                    if(status==1)
                    {
                        $('#otp').show();
                        showNotification("bg-black", "Please verify OTP", "top", "center",'animated zoomInUp', 'animated zoomOutUp');
                    }
                    else if(status==2){
                        $('#generate_otp').removeAttr('disabled');
                        $('#otp').hide();
                        showNotification("bg-black", "Please regenerate OTP", "top", "center",'animated zoomInUp', 'animated zoomOutUp');
                    }
                }
            });    
        });
        ...
        $('#verify_otp').click(function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    if(status==1)
                    {
                        window.location='../cumulative/dashboard';
                    }
                    else if(status==2){
                        $('#generate_otp').removeAttr('disabled');
                        $('#otp').hide();
